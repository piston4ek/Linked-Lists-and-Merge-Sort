...
void LinkedList<T>::insertOrdered(const T& newData) 
{
  ...
  // Otherwise, we will find "nieche place" between nodes
  else
  {
    Node* forInsert = new Node(newData);
    Node* cur = head_->next;
    // We run our cycle until we did not get a nullptr
    ...
    while(cur)     
    {
      if(forInsert->data < cur->data)
      {
        ...
        prevNode->next = forInsert;
        forInsert->prev = prevNode;
        forInsert->next = cur;
        cur->prev = forInsert;
        // Necessarily plus one to size 
        size_++;
        // Stop cycle
        break;
      }
